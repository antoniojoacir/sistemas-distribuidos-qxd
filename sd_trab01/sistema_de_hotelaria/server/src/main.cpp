...
private:
  ...
  void handleClient(int clientSocket, sockaddr_in client) {
    ...
    while (true) {
      ...
      try {
        if (args[0] == "res") {
          hospedagem.addReservation(args[1], +args[2],
                                    std::make_shared<Person>(args[3], args[4]));
        } else if (args[0] == "cancel") {
          hospedagem.rmReservation(args[1], +args[2], args[3]);
        } else if (args[0] == "show") {
          std::string bytesSend = hospedagem.show();
          std::cout << bytesSend;
          send(clientSocket, bytesSend.c_str(), bytesSend.length() + 1, 0);
        } else if (args[0] == "list") {
          std::string response = hospedagem.listarReservas(args[1]);
