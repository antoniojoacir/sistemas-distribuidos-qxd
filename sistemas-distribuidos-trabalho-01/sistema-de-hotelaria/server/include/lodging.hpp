...
struct Lodging {
  ...
  void cancelarReserva(int quarto, const std::string& cpf, const std::string& classe) {
        for (auto& reserva : reservas) {
            if (reserva.second->WhoIsIn(quarto) != "Empty") {
                reserva.second->CancelReservation(quarto, cpf);
                std::string arquivo = basePath + "/" + classe + "/" + cpf + ".bin";
                if (fs::exists(arquivo)) {
                    fs::remove(arquivo);
                }
                fn::write("Reserva cancelada para o quarto " + std::to_string(quarto));
                return;
            }
        }
        throw std::runtime_error("Reserva não encontrada.");
    }
